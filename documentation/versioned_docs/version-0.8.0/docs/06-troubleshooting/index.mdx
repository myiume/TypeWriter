...
## Road network nodes do not connect / pathfinding is not working

The current pathfinding algorithm is legacy and does not work at coordinates below 0 and above 256. Any road network nodes that are placed beyond those coordinates will also not connect and work.
If that is not the case, make sure you recalculate edges, so the nodes reconnect properly on your road network.
